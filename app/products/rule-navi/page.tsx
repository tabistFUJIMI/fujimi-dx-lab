...
const FEATURES = [
  { icon: "🔍", title: "AIチャット検索", description: "「有給って何日前に申請？」と聞くだけ。AIが就業規則やマニュアルから該当箇所を見つけて即座に回答します。キーワード検索では見つからない曖昧な質問にも対応。" },
  { icon: "📄", title: "PDF自動取り込み", description: "就業規則・業務マニュアル・ハンドブックのPDFをアップするだけ。AIが中身を読み取って検索可能な状態にします。複数ファイルの一括アップロードに対応。" },
  { icon: "🔗", title: "出典明示", description: "AIの回答には必ず「このドキュメントのこの部分に書いてあります」という出典が付きます。スタッフが自分で元の文書を確認できて安心。" },
  { icon: "❓", title: "FAQ自動生成", description: "よく聞かれる質問をAIが自動でFAQ化。新人教育の手間がぐっと減り、教える側の負担を軽減します。" },
  { icon: "📮", title: "匿名質問箱", description: "「聞きづらいこと」も匿名で質問OK。心理的安全性を高め、風通しの良い職場づくりをサポートします。" },
  { icon: "💓", title: "パルスチェック", description: "スタッフの声・組織の雰囲気を定期的に収集。離職兆候や組織の健康状態を可視化し、管理者が早期対応できます。" },
  { icon: "🏢", title: "理念・バリュー管理", description: "会社の理念や経営メッセージをAIに学習させ、文脈に応じて回答に反映。ミッション・ビジョン・バリューを日常的にスタッフに浸透させます。" },
  { icon: "🔐", title: "アカウント不要で利用", description: "スタッフは専用アクセスURLからアクセスするだけ。アカウント登録・ログイン操作なしで利用できるため、導入時の教育コストがほぼゼロ。" },
];
...
const PLANS = [
  ...
  {
    name: "スタンダード", price: "¥1,100", period: "/月", description: "少人数チームに", color: "#a855f7", bgColor: "#faf5ff",
    features: [
  ...
  {
    name: "プロ", price: "¥2,200", period: "/月", description: "しっかり活用", recommended: true, color: "#7e22ce", bgColor: "#faf5ff",
    features: [
      ...
      <main>
        <section className="relative min-h-[70vh] overflow-hidden px-4 pt-32 pb-20 text-white md:pt-40 md:pb-28" style={{ background: "linear-gradient(135deg, #a855f7 0%, #9333ea 50%, #7e22ce 100%)" }}>
          <div className="relative z-10 mx-auto max-w-6xl"><div className="max-w-2xl">
            ...
            <FadeIn delay={0.25}><div className="mt-8 flex flex-wrap gap-4"><SignupFlowButton label="申し込む" appName="Rule Navi" accentColor="#9333ea" className="inline-flex items-center gap-2 rounded-xl bg-white px-8 py-4 text-base font-bold shadow-lg transition-all duration-300 hover:scale-105" style={{ color: "#9333ea" }} /><a href="#pricing" className="inline-flex items-center gap-2 rounded-xl border-2 border-white/40 bg-white/10 px-8 py-4 text-base font-bold text-white backdrop-blur-sm transition-all duration-300 hover:bg-white/20">料金プランを見る</a></div></FadeIn>
          </div></div>
        </section>
        ...
        <section id="contact" className="relative overflow-hidden px-4 py-24 text-white md:py-32" style={{background:"linear-gradient(135deg, #a855f7 0%, #9333ea 50%, #7e22ce 100%)"}}>
          <div className="relative z-10 mx-auto max-w-3xl text-center">
            <FadeIn>
              ...
              <div className="mt-8">
                <SignupFlowButton label="申し込む" appName="Rule Navi" accentColor="#9333ea" className="inline-flex items-center gap-2 rounded-xl bg-white px-10 py-4 text-lg font-bold shadow-xl transition-all duration-300 hover:scale-105 active:scale-[0.98]" style={{ color: "#9333ea" }} />
                <p className="mt-3 text-xs text-purple-100/80">3ステップで完了（ボタンを押すと詳しい流れが表示されます）</p>
              </div>
