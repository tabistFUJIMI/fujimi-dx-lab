...
const MENUS = [
  { name: "全身もみほぐし 40分", price: "4,000円〜", popular: false },
  { name: "全身もみほぐし 60分", price: "6,000円", popular: true },
  { name: "全身もみほぐし 90分", price: "8,500円", popular: false },
  { name: "足つぼリフレクソロジー 30分", price: "3,000円", popular: false },
  { name: "ヘッドスパ 20分", price: "2,500円", popular: false },
  { name: "アロマトリートメント 60分", price: "7,000円", popular: false },
];

const STAFF = [
  { name: "指名なし（おまかせ）", extra: null, avatar: "👤" },
  { name: "セラピスト 佐野", extra: null, avatar: "🧑‍⚕️" },
  { name: "TOPセラピスト い", extra: "指名料 +1,000円", avatar: "⭐" },
];
...
function StepStaff() {
  return (
    <div className="space-y-2">
      <p className="mb-3 text-xs text-gray-500">担当者を選択してください</p>
      {STAFF.map((s) => (
        <div
          key={s.name}
        ...
        <div key={sec.label} className="mb-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {sec.slots.map((slot, i) => {
              const selected = slot === "10:00" && sec.label === "午前";
              ...
              return (
                <span
                  ...
                  className={`rounded-lg border px-2.5 py-1.5 text-xs ${
                    selected
                      ? "border-violet-400 bg-violet-600 font-bold text-white"
                      : disabled
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3 rounded-xl border border-gray-200 bg-gray-50 p-4">
        {[
          { label: "メニュー", value: "全身もみほぐし 60分" },
          { label: "料金", value: "¥6,000" },
          { label: "担当者", value: "指名なし（おまかせ）" },
          { label: "日付", value: "2026年4月1日（水）" },
          { label: "時間", value: "10:00 〜 11:00" },
        ].map((r) => (
      ...
      <button className="w-full rounded-xl bg-violet-600 py-3 text-sm font-bold text-white">
        この内容で予約する
      </button>
    </div>
  );
...
export default function MockLineBooking() {
      ...
      <div className="overflow-hidden rounded-[2.5rem] border-4 border-gray-800 bg-gray-800 shadow-2xl">
        ...
        <div className="bg-white">
          ...
          <div className="flex items-center justify-between bg-violet-700 px-4 py-3">
            ...
            <span className="text-[10px] text-violet-200">スマート予約管理</span>
          </div>
